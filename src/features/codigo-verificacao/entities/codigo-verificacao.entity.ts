...
export class CodigoVerificacaoEntity implements ICodigoVerificacao {
    ...
    // })
    @PrimaryGeneratedColumn('increment', { name: 'COD_CODIGO_VERIFICACAO_PESSOA', })
    codCodigoVerificacaoPessoa: number;

    @Column('varchar', { name: 'TXT_INTERESSADO', length: 255, nullable: false, })
    txtInteressado: string;

    @Column('varchar', { name: 'TXT_CNPJ_CPF', length: 14, nullable: false, })
    txtCnpjCpf: string;

    @Column('varchar', { name: 'TXT_CODIGO_SEGURANCA', length: 50, nullable: false, })
    txtCodigoSeguranca: string;

    @Column('date', { name: 'DT_EXPIRACAO', nullable: false, })
    dtExpiracao: Date;
}
